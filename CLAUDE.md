...
wacrm is multi-store, multi-channel: `account` is the network/tenant (unchanged), `stores` are locations within it, and `channel_connections` (one per WhatsApp number or Telegram bot) each belong to a store. A `conversation` is keyed by `(contact, connection)`, not `(contact, account)` — the same contact talking to two connections gets two conversations, unified in one inbox. Contact identity (phone, WhatsApp BSUID/username, Telegram chat id/username, …) is resolved through `contact_identities` (`lib/channels/identity.ts`), not a single column — `contacts.phone` stays for backward compat/display and is `''`/`null` when the contact has no phone-based identity. Credentials live in `channel_connection_credentials`, RLS'd to service-role only (`lib/channels/connections.ts`, `getConnectionCredentials`) — never select it with a user client.

**The core (`sendOutbound`/`ingestInbound`, automations, flows, AI auto-reply) never imports a channel-specific module.** Everything channel-specific goes through the `ChannelProvider` contract in `lib/channels/types.ts` (capabilities, config/credentials schemas, lifecycle `connect`/`disconnect`/`health`, inbound `resolveConnection`/`verify`/`parse`, outbound `resolveTarget`/`send`/`react?`/`typing?`) and a process-local registry (`lib/channels/registry.ts`, `getProvider`/`registerProvider`). `lib/channels/send.ts` (`sendOutbound`) and `lib/channels/ingest.ts` (`ingestInbound`) are the only entry points the rest of the app calls; they look the provider up by `connection.channel_type` and never branch on it themselves.

**To add a channel:** implement `ChannelProvider` under `lib/channels/providers/<type>/` (see `providers/whatsapp-cloud/` for the WhatsApp adapter over `lib/whatsapp/*`, kept behaviour-identical, or `providers/telegram/` for a channel built straight against the contract), register it in `providers/index.ts` (`registerBuiltinProviders`, idempotent — `registry.ts` itself never imports a concrete provider), and add a `descriptor` (form fields) if it needs a settings UI (`lib/channels/ui-registry.tsx`). Capabilities declared on the provider drive composer gating (`lib/channels/composer-capabilities.ts`), flow/automation step warnings (`lib/channels/step-capabilities.ts`) and error mapping (`ChannelError` codes) — nothing else needs to know a channel exists.

**`/api/v1` is pre-stable until the first client**, then frozen (a breaking change needs `v2`); see the banner in `docs/public-api.md` and `docs/mcp.md`.
