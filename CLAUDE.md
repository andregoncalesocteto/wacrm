...
**Docker.** `docker-compose.yml` is the app alone; `docker-compose.supabase.yml` is an overlay adding a trimmed copy of the official Supabase self-hosting stack (db, auth, rest, realtime, storage, imgproxy, Kong; no Studio/functions/analytics), plus a one-shot `migrate` service (`docker/supabase/migrate.sh`) that applies `supabase/migrations` and records them in `public.wacrm_migrations`. Non-obvious constraints:
