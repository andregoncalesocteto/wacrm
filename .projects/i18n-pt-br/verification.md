...
| 7 | Flows (lista, editor, execuções) | **Restante:** painel de validação do editor mostra as mensagens de `src/lib/flows/validate.ts` em inglês ("Pick an entry node before activating." etc., ~60 mensagens montadas na lib e usadas também pela API de ativação) | Nenhuma | limpo |
...
## Restante (impede `passes: true`)
- Mensagens de validação de fluxos (`src/lib/flows/validate.ts` e o painel `validation-panel.tsx`): exigem código + params
  por mensagem, ~60 chaves nos 4 catálogos e ajuste dos testes/da rota de ativação. Não coube nesta iteração.
