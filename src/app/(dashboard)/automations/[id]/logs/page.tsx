      ...
      try {
        ...
        const [autRes, logRes] = await Promise.all([
          ...
          supabase
            ...
            .select(
              `*, contact:contacts(id, name, phone, ${CONTACT_IDENTITIES_EMBED})`
            )
            .eq('automation_id', id)
        ...
        setLogs(
          (logRes.data ?? []).map((row) => {
            const contact = firstOf(
              row.contact as Parameters<typeof withIdentities>[0] | null
            );
            return {
              ...row,
              contact: contact ? withIdentities(contact) : undefined,
            };
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-foreground truncate text-sm font-medium">
                      {(log.contact && display.name(log.contact)) ||
                        t('unknownContact')}
                    </div>
