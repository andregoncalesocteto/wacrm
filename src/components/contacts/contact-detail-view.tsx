...
export function ContactDetailView({
  ...
  const fetchContact = useCallback(async () => {
    ...
    if (data) {
      setContact(withIdentities(data) as Contact);
  ...
  async function copyPhone() {
    ...
    await navigator.clipboard.writeText(
      display.secondary(contact).split(' · ')[0]
    );
    setCopiedPhone(true);
  ...
  async function saveDetails() {
    if (!contactId || (showPhoneField && !editPhone.trim())) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        // A contact without a WhatsApp number (Telegram) has no phone field:
        // leave the column untouched.
        ...(showPhoneField && { phone: editPhone.trim() }),
  ...
  // The phone is editable for anything WhatsApp-reachable (as before) and for
  // any contact that already has one; hidden for identity-only contacts.
  const showPhoneField = contact
    ? !!contact.phone?.trim() ||
      isWhatsAppReachable(contact, contact.identities)
    : true;
  const displayName = contact ? display.name(contact) : '';
  const secondaryLine = contact ? display.secondary(contact) : '';
