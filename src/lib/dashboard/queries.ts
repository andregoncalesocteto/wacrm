...
/** Feed label for a contact: name, else phone / @handle / chat id. */
function dashContactName(contact: DashContact | null | undefined): string {
  if (!contact) return '';
  return contactDisplayName(
    contact,
    identitiesFromRows(contact.contact_identities)
  );
}
...
export async function loadActivity(
  ...
  const [msgs, contacts, deals, broadcasts, autoLogs] = await Promise.all([
    db
      .from('messages')
      .select(
        'id, content_text, sender_type, created_at, conversation_id, conversations(contact_id, contacts(name, phone, contact_identities(kind, external_id, handle)))'
      )
    ...
    db
      .from('contacts')
      .select(
        'id, name, phone, created_at, contact_identities(kind, external_id, handle)'
      )
      .order('created_at', { ascending: false })
      .limit(10),
    db
    ...
    db
      ...
      .select(
        'id, trigger_event, status, created_at, automation:automations(name), contact:contacts(name, phone, contact_identities(kind, external_id, handle))'
      )
      .order('created_at', { ascending: false })
      .limit(10),
  ]);
  ...
  for (const m of (msgs.data ?? []) as unknown as Array<{
    ...
    conversations:
      | {
          ...
          contacts: DashContact[] | DashContact | null;
        }[]
      | {
          contact_id: string | null;
          contacts: DashContact[] | DashContact | null;
    ...
    const contact = Array.isArray(conv?.contacts)
      ? conv?.contacts[0]
      : conv?.contacts;
    const who = dashContactName(contact) || 'Unknown';
    items.push({
      id: `msg-${m.id}`,
      kind: 'message',
  ...
  for (const c of (contacts.data ?? []) as Array<
    DashContact & { id: string; created_at: string }
  >) {
    items.push({
      id: `contact-${c.id}`,
      kind: 'contact',
      text: `New contact: ${dashContactName(c)}`,
      at: c.created_at,
      href: '/contacts',
    });
  }
  ...
  for (const l of (autoLogs.data ?? []) as unknown as Array<{
    ...
    contact: DashContact[] | DashContact | null;
  }>) {
    const automation = Array.isArray(l.automation)
      ? l.automation[0]
      : l.automation;
    const contact = Array.isArray(l.contact) ? l.contact[0] : l.contact;
    const who = dashContactName(contact) || 'a contact';
